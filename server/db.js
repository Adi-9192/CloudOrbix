...
export function normalizeQueryForAzureSql(sqlText, values = []) {
  ...
  text = text.replace(/\bTO_CHAR\s*\(\s*(.+?)\s*,\s*'([^']+)'\s*\)/gi, (_, expression, format) => {
    const normalizedFormat = String(format).toUpperCase();
    if (normalizedFormat === "YYYY" || normalizedFormat === "YYYY-MM-DD") return `FORMAT(${expression}, 'yyyy')`;
    if (normalizedFormat.includes("MON") || normalizedFormat === "MM" || normalizedFormat === "MONTH") return `FORMAT(${expression}, 'MMM')`;
    if (normalizedFormat.includes("IW") || normalizedFormat.includes("IYYY")) return `FORMAT(${expression}, 'yyyy-MM-dd')`;
    return `FORMAT(${expression}, 'yyyy-MM-dd')`;
  });
  text = text.replace(
    /COUNT\(\s*\*\s*\)\s*FILTER\s*\(\s*WHERE\s+([^)]*?)\s*\)/gi,
    (_, condition) => `SUM(CASE WHEN ${condition} THEN 1 ELSE 0 END)`,
  );
  text = text.replace(
  ...
  text = text.replace(
    ...
    (_, agg, condition) => `COALESCE(STRING_AGG(DISTINCT ${agg}, ','), '')`,
  );
  text = text.replace(
    /COALESCE\(array_agg\(([^)]+?)\),\s*'\{\}'\)/gi,
    (_, agg) => `COALESCE(STRING_AGG(DISTINCT ${agg}, ','), '')`,
  );
  text = text.replace(
  ...
  try {
    ...
    console.warn(
      `Database connection failed (${error.number || "unknown"}): ${
        error.message || "Unable to connect to Azure SQL."
      }`,
    );
    throw error;
  }
...
export async function updateUserPassword(userId, password) {
  ...
  if (pool) {
    await executeQuery("UPDATE users SET password_hash = @p1 WHERE id = @p2", [
      hashedPassword,
      userId,
    ]);
    return;
...
`,
        [userEmail, action, oldValue ?? "—", newValue ?? "—"],
      );
    } catch (error) {
