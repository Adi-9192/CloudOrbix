...
async function findDbClient(clientId) { const result = await getPool().query(`SELECT ${columns} FROM clients c WHERE c.client_id=$1`, [clientId]); return result.rows[0] && mapRow(result.rows[0]); }
...
router.get('/approvals', protectRoute, requireRole('Admin'), async (req, res, next) => { try { const result = await getPool().query(`SELECT ${columns} FROM clients c WHERE c.approval_status='pending' ORDER BY c.updated_at DESC`); return res.json({ clients: result.rows.map(mapRow) }); } catch (error) { return next(error); } });
