...
router.get('/', protectRoute, requireRole('Admin', 'Manager', 'Operations Team'), async (req, res, next) => {
  try {
    ...
    const result = await pool.query(`SELECT id,user_email,action,old_value,new_value,created_at FROM audit_logs ORDER BY created_at DESC, id DESC OFFSET 0 ROWS FETCH NEXT 500 ROWS ONLY`);
